...
export const quotaReminderEmail = ({ name, unsubscribeUrl }) => ({
  ...
  html: layout(`
    ...
    <p>Need more analyses? The <strong>Solo plan at $9.9/month</strong> gives you 25 analyses and 3 AI comparisons per month — plus a 7-day free trial.</p>
...
export const proFeaturesEmail = ({ name, unsubscribeUrl }) => ({
  ...
  html: layout(`
    ...
    <div class="highlight">
      <p><strong>🔍 AI Contract Compare</strong> — Compare two versions of a contract side-by-side. See exactly what changed and whether the changes help or hurt you.</p>
    </div>
    <div class="highlight">
...
// ── Step 7 — The cost of one bad contract (Day 14) ───────────────────────────
export const upgradeOfferEmail = ({ name, unsubscribeUrl }) => ({
  subject: 'One bad contract can cost you everything',
  html: layout(`
    <h1>The real cost of signing blind</h1>
    <p>Hi ${name || 'there'},</p>
    <p>You've been with Signova for 2 weeks. Before we leave you to it, we want to share something important.</p>

    <div class="highlight">
      <p>A freelancer signed a contract with unlimited liability. One dispute later — they owed $40,000. The clause was buried in paragraph 14. It took 30 seconds to find with Signova.</p>
    </div>

    <p>Most contract disputes don't come from obvious traps. They come from clauses that <strong>sound reasonable</strong> but aren't — IP grabs, auto-renewals, one-sided termination rights.</p>

    <p>The free plan gives you 3 analyses. If you have more contracts — past, present, or incoming — the <strong>Solo plan ($9.9/month)</strong> covers 25 analyses, 3 AI comparisons, and stores everything in one place.</p>

    <ul class="step-list">
      <li><span class="step-num">→</span><span>Employment contracts</span></li>
      <li><span class="step-num">→</span><span>Client agreements</span></li>
      <li><span class="step-num">→</span><span>Software licenses you actually depend on</span></li>
      <li><span class="step-num">→</span><span>Leases, vendor agreements, NDAs</span></li>
    </ul>

    <p>Every contract you sign without reading it carefully is a risk you're accepting without knowing.</p>
