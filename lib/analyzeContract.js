 ...
 * @param {object} [options]      - { contractId, userId, supabase }
 * @returns {object}              - Full analysis result
 */
export const analyzeContract = async (contractText, userProfile, options = {}) => {
  const { contractId, userId, supabase } = options
  ...
  const extracted = parseExtractionResponse(extractionRaw)
  const confidenceWarnings = flagLowConfidence(extracted)
  ...
  const narrativePrompt = buildNarrativePrompt(extracted, contextResult, userProfile)
  const narrativeRaw = await callClaude(
    'claude-sonnet-4-6',
    narrativePrompt
  )
  const narrative = parseNarrativeResponse(narrativeRaw)
